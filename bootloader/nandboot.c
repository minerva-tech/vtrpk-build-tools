...
static Uint32 load_image (NAND_InfoHandle hNandInfo, Uint8 *rxBuf)
{
    Uint32 readError;
    Uint32 magicNum;
    Uint32 i, block, page;
...
static Uint32 find_image (NAND_InfoHandle hNandInfo, Uint32 start, Uint32 end, Uint8 *rxBuf)
{
    Uint32 i;
    Uint32 magicNum;
    ...
    for (i = start; i < end; i++) {
        ...
        if (NAND_readPage(hNandInfo, i, 0, rxBuf) != E_PASS) continue;
...
Uint32 NANDBOOT_copy (Uint32 mode)
{
    ...
    for (i = DEVICE_NAND_KERNEL_SEARCH_START_BLOCK; i < DEVICE_NAND_KERNEL_SEARCH_END_BLOCK; i++) {
        if (find_image(hNandInfo, i, DEVICE_NAND_KERNEL_SEARCH_END_BLOCK, rxBuf) && (load_image(hNandInfo, rxBuf) == E_PASS))
            return E_PASS;
    }

    // Never found valid header in any page 0 of any of searched blocks
    if (i >= DEVICE_NAND_KERNEL_SEARCH_END_BLOCK)
        DEBUG_printString("No valid boot image found!\r\n");

load_rescue_image:

    for (i = DEVICE_NAND_RESCUE_SEARCH_START_BLOCK; i < DEVICE_NAND_RESCUE_SEARCH_END_BLOCK; i++) {
        if (find_image(hNandInfo, i, DEVICE_NAND_RESCUE_SEARCH_END_BLOCK, rxBuf) && (load_image(hNandInfo, rxBuf) == E_PASS))
            return E_PASS;
    }

    // Never found valid header in any page 0 of any of searched blocks
    if (i >= DEVICE_NAND_RESCUE_SEARCH_END_BLOCK)
