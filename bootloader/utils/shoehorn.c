...
int main(int argc, char **argv)
{
    ...
    if (!image) {
        fprintf(stderr,"no 'image' is specified\n");
        return EXIT_FAILURE;
    }
    if (read_image()) return EXIT_FAILURE;
    ...
    printf("Send CRC32\n");
    for (i = 0; i < 256;) {
        snprintf(out_str,9,"%08x",crc32_table[i]);
        put_string(out_str,8);
        i++;
    }
    ...
    for (i = 0; i < image_size; i += 4) {
        p = (unsigned int *)(&ldr[i]);
        snprintf(out_str,9,"%08x",*p);
        put_string(out_str,8);
    }
